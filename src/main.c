...
main(int argc, char *argv[])
{
	...
	if (!pipeline || !source || !udpsink || !conv || !encoder_q || !rate || !encoder || !parser || !rtp || !rtpfec) {
	...
	srcstructure = gst_structure_new("video/x-raw",
					 ...
					 "width", G_TYPE_INT, VIDEO_W,
					 "height", G_TYPE_INT, VIDEO_H,
					 NULL);
	if (!srcstructure) {
	...
	ratecaps = gst_caps_new_simple("video/x-raw", "framerate", GST_TYPE_FRACTION, VIDEO_FPS, 1, NULL);
	g_object_set (G_OBJECT (ratec), "caps", ratecaps, NULL);
	gst_caps_unref (ratecaps);

	/* Modify the source's properties */
	g_object_set (conv,	"flip-method", 2, NULL);
	g_object_set (encoder,	"bitrate", BITRATE, /*"iframeinterval", 60, "preset-level", 0,*/ "control-rate", 1,"maxperf-enable", true, "MeasureEncoderLatency", true, NULL);
	g_object_set (rtp,	"config-interval", 1, "mtu", 1420, "pt", 96, NULL);
	g_object_set (rtpfec,	"percentage", FEC_PERCENT, "pt", 122, NULL);
	g_object_set (udpsink,	"host", "192.168.50.100", "port", 5600, "sync", false, "async", false, NULL);

	/* Build the pipeline */
	gst_bin_add_many (GST_BIN (pipeline), source, caps, conv, /*encoder_q, */ /*rate, ratec,*/ encoder, parser, rtp, rtpfec, udpsink, NULL);
	if (gst_element_link_many (source, caps, conv, /*encoder_q,*//* rate, ratec,*/ encoder, parser, rtp, rtpfec, udpsink, NULL) != TRUE) {
